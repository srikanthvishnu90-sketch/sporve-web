...
(function () {
  ...
  function whatIsPage(meta) {
    var cells = [
      ...
      ["Payments", "Families pay the listed service price without an added booking fee. The coach sees gross revenue, Sporv's twelve-percent platform fee, and net payout on one record. A refund keeps the original charge and adjustment visible. No transfer is reduced to an unexplained balance."],
  ...
  function paymentsPage(meta) {
    var prose = [
      "A family pays the price the coach published. Sporv does not add a family booking fee at checkout. The paid booking records the gross service price, and the coach side records the platform fee and resulting net payout. Keeping incidence explicit matters: the parent should not discover a surcharge at the last step, and the coach should not have to reverse-engineer what arrived. Tax or processor information, when applicable, is labeled separately rather than folded into the platform fee, and no estimate is presented as a settled transfer.",
      "The platform fee is a flat twelve percent of the paid booking. It is taken only when there is paid revenue, then itemized on the payout record as gross, fee, and net. The coach connects a payout account after the required onboarding and safety gates clear. Payout state moves from pending to in transit to paid, so a balance never disappears behind a generic success message. A failed or paused transfer keeps its amount and status while the coach follows the remediation path; it does not masquerade as revenue that reached the bank.",
      "Refunds preserve the same arithmetic. When money goes back under the saved cancellation policy, Sporv returns the corresponding portion of its fee instead of keeping a fee on refunded coach revenue. The original charge, refunded amount, remaining paid amount, returned fee, and payout adjustment stay visible. That trail lets both sides distinguish a pending transfer from money that was actually deposited. Partial refunds use the paid amount actually returned, so several adjustments still reconcile to the original booking instead of producing a second unexplained ledger entry."
    ];
    ...
    var worked = "<div class='pg-worked-math'><code>gross       $45.00\nfee      ×     .12\n-----------------\nplatform   −$5.40\ncoach net   $39.60</code>" +
      "<p data-prose>A forty-five-dollar lesson produces a five-dollar-and-forty-cent fee and a thirty-nine-dollar-and-sixty-cent coach payout. The same formula applies to every paid booking; there is no tier to infer and no separate invoice after the fact. The rail repeats those exact amounts in the transfer record the coach can reopen later. These are disclosed demo amounts, but the live calculation is written from the booking's stored gross value rather than copied from a marketing example. Rounding is resolved once on that record, so the displayed parts always reconcile.</p></div>";
