...
| 17 | **The flat-12% fee fix is half-finished — the coach's *displayed* fee was left on the retired 18/4 schedule** (*pentest 2026-08-08*). PR #14 unifies the **charge** to a flat 12% (`stripe-create-checkout` default `?? 1200`, deletes `first_booking`/`recurring` rows, drops `resolve_platform_fee_bps`) but **does not touch `lib/core/utils/platform_fee.dart`** (`kFirstBookingFeeBps=1800` / `kRecurringFeeBps=400`), which feeds the coach earnings page, CSV export, and 1099 projection via `provider_controller._itemize`. After PR #14 merges, Stripe withholds 12% while the app shows 18%/4% — a trust + tax-reporting mismatch on the exact surface coaches use to reconcile income. Not a money-movement hole (the charge is correct); the comment at `platform_fee.dart:11-13` still falsely claims the fee is "derived from `resolve_platform_fee_bps`". Root cause: the rake is authored in three places (edge env, DB table, Dart constants) and PR #14 only unified two. | When PR #14 merges, does the number on the coach's earnings/CSV/1099 equal the 12% Stripe actually deducts — and should the app display the *charged* fee mirrored onto the booking rather than any client-side projection at all? |
